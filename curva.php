<?php
session_start();
require_once 'includes/db_connect.php'; // Il ponte con PostgreSQL ($db)
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['testo_messaggio']) && isset($_SESSION['id_utente'])) {
    ...
    if (!empty($testo)) {
        // Prepariamo la query con $1 e $2 invece dei ?
        $sql = "INSERT INTO messaggi (id_utente, testo) VALUES ($1, $2)";
        pg_prepare($db, "ins_msg", $sql);
        $res = pg_execute($db, "ins_msg", array($_SESSION['id_utente'], $testo));
...
if (isset($_POST['elimina_messaggio']) && isset($_SESSION['ruolo']) && $_SESSION['ruolo'] === 'admin') {
    ...
    $sql_del = "DELETE FROM messaggi WHERE id = $1";
    pg_prepare($db, "del_msg", $sql_del);
    pg_execute($db, "del_msg", array($id_da_eliminare));
...
if (isset($_SESSION['id_utente'])) {
    ...
    // Usiamo pg_query per una select semplice senza parametri
    $res_msg = pg_query($db, $sql_messaggi);
    if ($res_msg) {
        $messaggi = pg_fetch_all($res_msg); // Recupera tutte le righe come array
        if ($messaggi === false) $messaggi = []; // Se non ci sono messaggi, inizializza array vuoto
    }
}
    ...
    <section class="la-curva">
        ...
        // SE L'UTENTE È LOGGATO
            ...
            if (isset($_SESSION['ruolo'])) {
                ...
                } elseif ($_SESSION['ruolo'] === 'super_tifoso') {
            ...
            echo "<a href='login.php?logout=true' class='btn-logout' style='color:red; text-decoration:none; font-weight:bold;'>Esci dallo stadio (Logout)</a>";
            echo "</div>";
            
            // Bacheca Messaggi
            echo "<div class='bacheca-messaggi' style='margin-top: 40px;'>";
            echo "<h3 class='titolo-chiaro' style='color:white;'>Ultimi cori</h3>";
            
            if (count($messaggi) > 0) {
                foreach ($messaggi as $msg) {
                    echo "<article class='messaggio' style='background: white; padding: 15px; margin-bottom: 15px; border-radius: 8px;'>";
                    echo "<header class='autore-info'>";
                    echo "<strong>" . htmlspecialchars($msg['nome']) . "</strong> ";
                    
                    // Badge in base al ruolo
                    if ($msg['ruolo'] === 'admin') {
                        echo "<span class='badge admin' style='background:#ffd700; padding:2px 5px; border-radius:4px;'>🛡️ Admin</span>";
                    } else {
                        echo "<span class='badge tifoso' style='background:#ddd; padding:2px 5px; border-radius:4px;'>⚽ Tifoso</span>";
                    }
                    echo " <small style='color:gray;'>" . $msg['data_invio'] . "</small>";
                    echo "</header>";
                    
                    echo "<p class='testo-messaggio' style='margin-top:10px; font-size:1.1em;'>" . htmlspecialchars($msg['testo']) . "</p>";
